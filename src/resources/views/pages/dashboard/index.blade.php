        ...
        <section class="flex justify-center w-full">
            ...
            <div class="flex flex-col w-full space-y-6 text-gray-700">
                <div class="grid w-full grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
                    @foreach ($data_view['departments_core'] as $department)
                        <div id="card-total-{{ $department->id }}" class="flex justify-center w-full">
                            <div
                                    ...
                                    <div class="p-1 mt-14">
                                        <ol class="flex items-center justify-center h-full list-reset">
                                            @foreach ($status as $val)
                                                <li>
                                                    <a href="{{ route('tickets.index', ['status' => strtolower($val['nama']), 'department' => str_replace(' ', '-', strtolower($department->id))]) }}"
                ...
                <div class="grid w-full grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
                    @foreach ($data_view['departments'] as $department)
                        <div id="card-mttr-{{$department->id}}"
                            ...
                            <div
                                ...
                                <div class="p-6">
                                    ...
                                    <canvas
                                        id="chart-mttr-month-{{$department->id}}"></canvas>
                                    <div
                                        id="current_mttr_month{{$department->id}}">
                                    </div>
                                </div>
                            </div>
                        </div>
                                ...
                                <div
                                    ...
                                    <div class="p-6">
                                        <div class="min-h-[80px]">
                                            <h5 class="px-4 mb-2 text-xl font-medium text-gray-800"><span
                                                    class="block">MTTRecovery</span>
                                                {{ str_replace('Cyber Security', 'IT & Cyber Security', $department->name) }}
                                            </h5>
                                        </div>
                                        <canvas
                                            id="chart-mttr-week-{{$department->id}}"></canvas>
                                        <div
                                            id="current_mttr_week{{$department->id}}">
                                        </div>
                                    </div>
                                </div>
                            </div>
...
@push('scripts')
    ...
    <script>
        ...
        async function getDepartmentByType(type) {
            try {
                return await apiCall({
                    url: '{{ route('departments.get-by-tipe') }}',
                    data: {
                        type: type
                    },
        ...
        async function initTotal() {
            ...
            const listDepartment = await getDepartmentByType('semicore');

            if (listDepartment.data.length == 0) {
                return
            }
            for (const department of listDepartment.data) {
                ...
                try {
                    ...
                    const cardDepartment = $(`#card-total-${departmentId}`)
                ...
                try {
                    ...
                    let dataMTTA = {
                        'mttr_month': {
                            ...
                            'XTitle': 'Month',
                            'YTitle': 'Recovery',
                            'id1': `chart-mttr-month-${departmentId}`,
                            'id2': `current_mttr_month${departmentId}`
                        },
                    }
                    ...
                    if (departmentId == 2) {
                        dataMTTA = {
                            ...
                            'mttr_week': {
                                ...
                                'XTitle': 'Week',
                                'YTitle': 'Recovery',
                                'id1': `chart-mttr-week-${departmentId}`,
                                'id2': `current_mttr_week${departmentId}`
                            },
                        }
                    }
