            ...
            <tbody>
                @foreach ($data as $devices)
                    <tr>
                        ...
                        <td>{{ $devices->sn ?? '' }}</td>
                        <td><span
                                class="rounded text-white {{ $devices->status == '1' ? 'bg-green-0' : 'bg-grey-2' }} py-1 px-2">{{ $devices->status ? 'Active' : 'NonAktive' }}</span>
                        </td>
                        <td>
                            <div class="flex flex-row space-x-2 justify-center">
                                @if (($permission['role'] ?? null) == 'Admin' || in_array('Update Device', $submenu_middleware))
                                    <a href="{{ route('devices.edit', $devices->id) }}"
                                ...
                                @endif
                                @if (($permission['role'] ?? null) == 'Admin' || in_array('Delete Device', $submenu_middleware))
                                    <form action="{{ route('devices.delete', $devices->id) }}" method="POST" class="delete">
                                        @method('DELETE')
                                        @csrf
                                        <button type="submit"
        ...
        <div class="relative w-full h-full max-w-md md:h-auto">
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                <div class="p-6 text-center">
                    ...
                    <form action="{{ route('devices.export') }}" class="form-filter">
