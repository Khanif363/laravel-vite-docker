...
@section('content')
    ...
    <section class="w-full h-full mt-4 mb-10 overflow-x-auto">
        <table class="w-full text-sm text-center text-gray-600" id="dataTable">
            ...
            <tbody>
                @foreach ($data as $resumes)
                    <tr>
                        <td>{{ $loop->iteration }}</td>
                        <td>{{ $resumes->department->name ?? '' }}</td>
                        <td>{{ $resumes->name }}</td>
                        <td><span
                                class="rounded text-white {{ $resumes->status == '1' ? 'bg-green-0' : 'bg-grey-2' }} py-1 px-2">{{ $resumes->status ? 'Active' : 'NonAktive' }}</span>
                        </td>
                        <td>
                            <div class="flex flex-row space-x-2 justify-center">
                                @if (($permission['role'] ?? null) == 'Admin' || in_array('Update Resume', (array) $submenu_middleware))
                                    <a href="{{ route('resumes.edit', $resumes->id) }}"
                                ...
                                @endif
                                @if (($permission['role'] ?? null) == 'Admin' || in_array('Delete Resume', $submenu_middleware))
                                    <form action="{{ route('resumes.delete', $resumes->id) }}" method="POST" class="delete">
                                        @method('DELETE')
                                        @csrf
                                        <button type="submit"
        ...
        <div class="relative w-full h-full max-w-md md:h-auto">
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                <div class="p-6 text-center">
                    ...
                    <form action="{{ route('resumes.export') }}" class="form-filter">
