            ...
            <tbody>
                @foreach ($data as $services)
                    <tr>
                        ...
                        {{-- <td>{{ $services->department->name ?? '' }}</td> --}}
                        <td>{{ $services->name }}</td>
                        <td><span
                                class="rounded text-white {{ $services->status == '1' ? 'bg-green-0' : 'bg-grey-2' }} py-1 px-2">{{ $services->status ? 'Active' : 'NonAktive' }}</span>
                        </td>
                        <td>
                            <div class="flex flex-row space-x-2 justify-center">
                                @if (($permission['role'] ?? null) == 'Admin' || in_array('Update Service', (array) $submenu_middleware))
                                ...
                                @endif
                                @if (($permission['role'] ?? null) == 'Admin' || in_array('Delete Service', $submenu_middleware))
                                    <form action="{{ route('services.delete', $services->id) }}" method="POST" class="delete">
                                        @method('DELETE')
                                        @csrf
                                        <button type="submit"
        ...
        <div class="relative w-full h-full max-w-md md:h-auto">
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                <div class="p-6 text-center">
                    ...
                    <form action="{{ route('services.export') }}" class="form-filter">
