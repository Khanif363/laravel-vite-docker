...
@section('content')
    ...
    <section class="mx-20 md:mx-32 xl:mx-72">
        <div class="p-8 md:px-8 md:py-10 bg-[#F4F4F4] rounded-xl">
            <form id="form-ajax" action="{{ route('users.update', $user) }}" method="post">
              ...
              focus:invalid:border-pink-500 focus:invalid:ring-pink-500 text-gray-600"
                    ...
                    <div class="flex md:flex-row flex-col space-y-2 items-center">
                        <label for="password-edit" class="text-gray-600 w-full text-left">Password</label>
                        <div class="w-full">
              ...
              invalid:border-pink-500 invalid:text-pink-600
              focus:invalid:border-pink-500 focus:invalid:ring-pink-500 text-gray-600">
                            <label class="text-sm text-gray-500">Isi jika ingin menganti password</label>
                            <label id="password-error" class="error text-sm text-red-500" for="password-edit"></label>
                        </div>
                    </div>
                    <div class="flex md:flex-row flex-col space-y-2 items-center">
                            ...
                            <select name="department_id" type="text" class="w-full" id="department_id" required>
                                ...
                                @foreach ($departments as $item)
                                    <option value="{{ $item->id }}"
                                        {{ ($item->id ?? '') === ($user->department_id ?? null) ? 'selected' : '' }}>
                                        {{ $item->name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="flex justify-end w-full flex-col">
                            <select name="role_id" type="text" class="w-full" id="role_id" required>
                                ...
                                @forelse ($roles as $item)
                                    <option value="{{ $item->id ?? '' }}"
                                        {{ ($item->id ?? '') === ($user->role_id ?? null) ? 'selected' : '' }}>
                        ...
                        <div class="flex justify-end w-full flex-col">
                            <div class="flex items-center">
                                <input id="is_enable" name="is_enable" type="checkbox" value="1"
                                    class="w-4 h-4 rounded-md scale-75" {{ $user->is_enable ? ' checked' : '' }}>
                            </div>
                        </div>
                    </div>
                            ...
                            @forelse ($accessRights as $i => $item)
                                <div class="flex items-center">
                                    <input id="permission-{{ $item->id ?? '' }}" name="permissions[]" type="checkbox"
                                        ...
                                        data-not-find="true"
                                        {{ $user->accessRights->contains('id', $item->id) ?? false ? 'checked' : '' }}>
